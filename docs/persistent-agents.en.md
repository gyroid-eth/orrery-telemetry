...
## Applying the contract to another Mail instance or Air

Each machine uses its own local Mail and connection profile. On Air, in an Air terminal:

1. run `inspect` with the Air connection profile
2. verify and pin the Air Mail `server_instance_id`
3. select the Air-side numeric agent ID in the Air project
4. run `claim` or `recover` on Air if its state requires it
5. create and start an Air-specific persistent profile

Do not copy a Mac token, pending file, or receipt to Air. A Mac numeric ID need not identify the same row on Air. Identical hostnames, ports, URLs, or socket paths do not make two Mail instances identical; the persistent instance UUID is authoritative.
