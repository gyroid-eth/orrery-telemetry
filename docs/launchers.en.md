...
A fresh Codex child launch records `launch_origin: child` and its selected `codex_mcp_profile` in the launch expectation. After the official `SessionStart` verifies identity and rollout, that non-secret provenance is copied into the bound session receipt with the numeric agent ID, project, and provider. The receipt survives normal cleanup of child state, token, and isolated home, so the dashboard can distinguish a cleaned-up child from an unmanaged Codex session. A receipt created before provenance existed is not guessed to be a child from its name or history and cannot resume. At this stage normal cleanup still deletes the credential, so even a verified child reports `credential_missing`; resume retention adds credential preservation and fresh-home regeneration.

When the launcher created a child-specific `CODEX_HOME`, dashboard resume restores the same `CODEX_HOME`, `CODEX_SHARED_CODEX_DIR`, and writable root only after matching the formal project, agent ID and name with private child state / token and the local ORRERY proxy identity in `config.toml`. A same-name child home with no state, or a proxy for a different identity, is never adopted by guess and stops the resume.
