...
1. header (wordmark, DECK / NETWORK toggle, the `29 / 29 AGENTS` statistic, MAIL health, the LEFT pill, search, history range, NEW AGENT, and on NETWORK the SETTINGS entry)
...
- The header is sticky, minimum 58px, padding 9px 22px, 22px between items. The statistic stacks "number → label → 2px underline"; running and registered agents share one instrument, `29 / 29`, and the underline's width is the running ratio (standby is registered minus running, so it gets no instrument of its own). The DECK / NETWORK toggle, MAIL, and LEFT are pills (999px radius, hairline; the ground is `--panel` at 74% for the toggle and 62% for MAIL and LEFT). Search is 226px wide. NEW AGENT is amber at 22% with a 32% amber border, 7px radius, amber text
