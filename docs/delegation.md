...
`AGENTSTACK_MAX_RUNNING_AGENTS` を設定すると、`/delegate` の child も親と合計して上限に数えます。
たとえば上限を `2` にすると、親 1 体が動いている間に child は 1 体までです。

上限に達している場合、`/delegate` は child を登録したり tmux session を作ったりせず、上限に達した理由を表示して止まります。
待ち行列や、別の仕組みへの自動切替はしません。
値の選び方と、dashboard の RESUME も含む詳しい数え方は[設定の「同時に動かす agent 数」](configuration.md#同時に動かす-agent-数)を参照してください。
