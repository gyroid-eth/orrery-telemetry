...
- **Project key:** Shared `project-context.sh` retains explicit selection in an established session; otherwise it prefers the canonical Git repository containing hook-input cwd. Linked worktrees share a project, and different repositories remain separate. Outside Git it falls back through `AGENTSTACK_PROJECT_KEY`, `PROJECT_KEY`, installed `env.sh`, then cwd. Installed exports are read literally without executing shell code. The registration guard, SessionStart, child cleanup, and `agentstack-await-reply` share this resolver. Git protected roots follow the actual worktree and canonical repository; reservation paths are repository-relative.
...
It uses event watching when `fswatch` is available and two-second polling otherwise. Runtime delivery state and a short lease suppress duplicate injection of the same `(project, agent, message)`. Successful delivery removes per-message signals and preserves legacy server-owned dirty bits. A periodic scan every 30 seconds recovers missed events.

The delivery target must exactly match the agent name and belong to the signal's canonical project. New signals include the canonical key; legacy project slugs are resolved through the ORRERY Mail project resource. The session project is verified from tmux context or runtime metadata containing its project. An unknown or mismatched project leaves the signal pending without capturing the pane or injecting a notification. After sending notification text literally, it submits with a separate `C-m` call, avoiding bare shells. tmux calls run in timeout-controlled workers so a server stall cannot stop the entire watcher.
