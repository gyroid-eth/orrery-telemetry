...
`record-codex-session-index.py` が identity に使う runtime 値は SessionStart stdin の `session_id` だけです。`CODEX_THREAD_ID`、`CODEX_SESSION_ID`、cwd、時刻、候補数は使いません。payload path の実体が通常 file で、先頭 `session_meta` ID が一致し、launch metadata の project・数値 ID・name・program と launcher がこの process に渡した `launch_id` がすべて一致したときだけ `provider: codex` の receipt を書きます。child provenance は launcher が `child` と有効な MCP profile の組を明示した場合だけコピーし、片方だけの metadata は invalid binding として拒否します。receipt は通常 cleanup が削除する state / token / child home の外に残るため、dashboard は cleanup 済み child と unmanaged session を名前から推測せず区別できます。provenance 導入前の receipt は child に昇格しません。recorder は trusted plugin runner と同梱し、index の runtime root は launch metadata path から導出します。login shell が親の `AGENTSTACK_RUNTIME_DIR` を復元しても保存先をすり替えられません。内蔵 subagent event は別 CLI process ではなく root ID を共有するため除外します。
