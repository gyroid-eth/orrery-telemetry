...
`agent-start-codex` sessions receive `AdjectiveScientist` names such as
`SwiftBohr`; no migration is needed. Names are generated by
...
agent-mail using an explicit `AdjectiveScientist` name such as `SwiftBohr`;
...
agent-mail using an explicit `AdjectiveScientist` name before starting Codex.
...
`AdjectiveScientist` `AGENT_NAME`, just without shell-side registration.
