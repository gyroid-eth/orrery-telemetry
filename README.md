...
`agent-start-codex` sessions receive scientist names; no migration is needed.
Scientist names are generated by claude-agent-stack itself, so updating the
agent-mail upstream clone is not required for this feature. Running sessions
keep the launcher code they started with; restart those Claude/Codex sessions
to pick up new launcher behavior. This upgrade does not rely on removed payload
files, so no stale-file cleanup is needed.
