...
class QuotaSnapshot:
    ...
    def to_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {"provider": self.provider, "status": self.status, "source": self.source, "observed_at": self.observed_at, "buckets": [bucket.to_dict() for bucket in self.buckets]}
        if self.reason:
