...
- `set_contact_policy` が `registration_token` を受け取ります。渡されたときは登録済みの token と照合し、一致しなければ policy を変えずに固定の error を返します。渡さない場合は従来どおりです（認証は緩めていません）
...
- 回帰テスト: canary を値・引数名・dict の key のそれぞれに置き、未知の引数・型不正・入れ子で client 応答と server log の両方に現れないこと、`set_contact_policy` の正 token / 誤 token（非 ASCII 含む）/ token なしの挙動
