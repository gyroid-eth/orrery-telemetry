...
async def complete_system_user(
    ...
    *,
    ...
    return await _complete_system_user(
        system,
        user,
        model=model,
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if exc.errno == errno.EMFILE:
                    ...
                    fd_before_cleanup = get_fd_diagnostics()
                    cleared = clear_repo_cache()
                    fd_after_cleanup = get_fd_diagnostics()
                    wrapped_exc = ToolExecutionError(
                        "RESOURCE_EXHAUSTED",
                        ...
                        data={
                            ...
                            "freed_repos": cleared,
                            "error_detail": str(exc),
                            "fd_before_cleanup": fd_before_cleanup,
                            "fd_after_cleanup": fd_after_cleanup,
                        },
                    )
                else:
                    wrapped_exc = ToolExecutionError(
...
def _agent_to_dict(agent: Agent) -> dict[str, Any]:
    d: dict[str, Any] = {
        ...
        # `model` is normalized at read time so the deck/roster shows one
        # consistent spelling; `model_raw` preserves exactly what the agent
        # registered (the database is never rewritten). See model_normalize.py.
        "model": normalize_model(agent.model),
        "model_display": display_model(agent.model),
        "model_raw": agent.model,
...
def _resolve_registration_token(
    ...
    if requested_token is not None:
        ...
        if len(token) > 64:
            raise ValueError("registration_token must be 64 characters or fewer")
        if existing_token and not hmac.compare_digest(existing_token, token):
...
def _looks_like_unix_username(value: str) -> bool:
    ...
    if v.islower() and v.isalnum() and 2 <= len(v) <= 16:
        ...
        from agentstack_mail.utils import ADJECTIVES, NOUNS
...
async def _generate_unique_agent_name(
    ...
    if name_hint:
        ...
        if sanitized:
            ...
            # In passthrough mode, accept any sanitized name without format validation.
            if mode == "passthrough" or validate_agent_name_format(sanitized):
                if not await available(sanitized):
                    # In strict/passthrough mode, indicate conflict; in coerce, fall back to generation
                    if mode in ("strict", "passthrough"):
...
async def _get_or_create_agent(
    ...
    *,
    ...
    # 2. AGENTSTACK_MAIL_WINDOW_ID set + window identity exists -> use window display_name
    # 3. AGENTSTACK_MAIL_WINDOW_ID set + window identity NOT in DB -> create new, use generated name
    ...
    if mode == "always_auto" and not window_uuid:
        ...
        if not _validate_window_uuid(window_uuid):
            logger.warning("AGENTSTACK_MAIL_WINDOW_ID is not a valid UUID: %s", window_uuid)
            desired_name = await _generate_unique_agent_name(project, settings, None)
        else:
...
def _same_agent_exact_file_reservation(
    ...
    if existing.exclusive != candidate_exclusive:
        return False
    return _normalize_pathspec_pattern(existing.path_pattern) == _normalize_pathspec_pattern(candidate_path)


async def _ensure_file_reservation_min_ttl(
    reservation: FileReservation,
    ttl_seconds: int,
) -> FileReservation:
    ...
    async with get_session() as session:
        ...
        if db_reservation is None:
            raise ValueError(f"Reservation {reservation.id} no longer exists.")
        current_expiry = db_reservation.expires_ts
        if current_expiry < target_expiry:
            db_reservation.expires_ts = target_expiry
...
def _file_reservations_conflict(existing: FileReservation, candidate_path: str, candidate_exclusive: bool, candidate_agent: Agent) -> bool:
    ...
    def _normalize(p: str) -> str:
        import unicodedata
        return unicodedata.normalize("NFC", p.replace("\\", "/").lstrip("/"))
...
def _normalize_pathspec_pattern(pattern: str) -> str:
    """Normalize a pattern for PathSpec caching (slash normalization + leading slash strip + Unicode NFC)."""
    if _is_virtual_namespace(pattern):
        return pattern  # Preserve virtual namespace scheme
    import unicodedata
    return unicodedata.normalize("NFC", pattern.replace("\\", "/").lstrip("/"))
...
def build_mcp_server() -> FastMCP:
    ...
    async def register_agent(
        ...
        # It was leaking into JSONL conversation logs as plaintext (1554+ occurrences).
        # No hooks depend on this field (mark-agent-registered.sh reads 'name' only).
        # Enrich with window identity info if AGENTSTACK_MAIL_WINDOW_ID is set.
    ...
    async def create_agent_identity(
        ...
        # Update attachments policy and persist caller-supplied token when requested.
        token = _resolve_registration_token(None, registration_token)
        async with get_session() as session:
            db_agent = await session.get(Agent, agent.id)
            if db_agent:
                db_agent.attachments_policy = ap
        ...
        result = _agent_to_dict(agent)
        # NOTE: registration_token intentionally omitted from response (security).
        return result
    ...
    async def file_reservation_paths(
        ...
        async with _archive_write_lock(archive):
            ...
            for path in paths:
                ...
                reused_reservation: Optional[FileReservation] = None

                for file_reservation_record, _holder_name in existing_reservations:
                    if _same_agent_exact_file_reservation(file_reservation_record, path, exclusive, agent):
                        reused_reservation = await _ensure_file_reservation_min_ttl(
                            file_reservation_record,
                            ttl_seconds,
                        )
                        file_reservation_record.expires_ts = reused_reservation.expires_ts
                        break

                if reused_reservation is not None:
                    ...
                    granted.append(
                        {
                            "id": reused_reservation.id,
                            "path_pattern": reused_reservation.path_pattern,
                            "exclusive": reused_reservation.exclusive,
                            "reason": reused_reservation.reason,
                            "expires_ts": _iso(reused_reservation.expires_ts),
                        }
                    )
                    continue
                ...
                if conflicting_holders:
                    # Blocking model: deny reservation when exclusive conflict exists
    ...
    async def release_file_reservations_tool(
        ...
        try:
            ...
            async with get_session() as session:
                ...
                # Filter by paths using the same symmetric overlap semantics as renew.
                if paths:
                    reservations = [
    ...
    async def renew_file_reservations(
        ...
        async with get_session() as session:
            ...
            # Note: paths filtering is done post-query with fnmatch to support glob patterns
            result = await session.execute(stmt)
            file_reservations: list[FileReservation] = list(result.scalars().all())

        # Filter by paths using the same symmetric overlap semantics as release.
        if paths and file_reservations:
            filtered = []
            for fr in file_reservations:
                for p in paths:
                    if _patterns_overlap(fr.path_pattern, p):
                        filtered.append(fr)
                        break
            file_reservations = filtered
...
def _apply_tool_filter(mcp: CompatibilityFastMCP, settings: Settings) -> None:
    ...
    to_remove: list[str] = []
    for tool_name in sorted(mcp.published_tool_names):
        cluster = TOOL_CLUSTER_MAP.get(tool_name, "unclassified")
    ...
    if to_remove:
        ...
        logger.info(
            ...
            f"{len(mcp.published_tool_names) - len(to_remove)} tools exposed"
        )
