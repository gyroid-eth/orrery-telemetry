...
class _LRURepoCache:
    ...
    def __init__(self, maxsize: int = 16, eviction_grace_seconds: float = 60.0) -> None:
        self._maxsize = max(1, maxsize)
        self._eviction_grace_seconds = max(0.0, eviction_grace_seconds)
    ...
    def _cleanup_evicted(self, *, force: bool = False) -> int:
        ...
        for repo, evicted_at in self._evicted:
            age = now - evicted_at
            if force or age >= self._eviction_grace_seconds:
...
# Defaults are intentionally conservative because high agent parallelism mostly
# targets the same archive repo and open file pressure is more damaging than a
# small amount of extra Repo() churn.
_REPO_CACHE: _LRURepoCache = _LRURepoCache(
    maxsize=max(1, _RUNTIME_SETTINGS.repo_cache_maxsize),
    eviction_grace_seconds=max(0.0, _RUNTIME_SETTINGS.repo_eviction_grace_seconds),
)
...
_REPO_SEMAPHORE_LIMIT: int = max(1, _RUNTIME_SETTINGS.repo_operation_semaphore_limit)
_FD_HEADROOM_THRESHOLD: int = max(16, _RUNTIME_SETTINGS.repo_fd_headroom_threshold)
...
def get_fd_diagnostics() -> dict[str, Any]:
    """Return lightweight FD and repo-cache diagnostics for error reporting."""
    current, limit = get_fd_usage()
    return {
        "fd_current": current,
        "fd_limit": limit,
        "fd_headroom": get_fd_headroom(),
        "repo_cache": get_repo_cache_stats(),
        "repo_semaphore_limit": _REPO_SEMAPHORE_LIMIT,
        "fd_cleanup_threshold": _FD_HEADROOM_THRESHOLD,
    }


def proactive_fd_cleanup(*, threshold: int | None = None) -> int:
    ...
    """
    threshold = _FD_HEADROOM_THRESHOLD if threshold is None else max(1, threshold)
    ...
    if new_headroom >= 0 and new_headroom < max(1, threshold // 2):
