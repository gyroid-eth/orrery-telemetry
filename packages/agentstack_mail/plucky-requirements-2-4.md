# #2 / #4 fixed requirements — PluckyEinstein WIP checkpoint

Canonical source: AgentMail #8334, #8335, #8340. Status vocabulary is exactly `済 / 未 / 検証不能`.
...
3. **未** — Code now binds candidate SHA/module/generator, UTC interval, canonical paths, generator+seed provenance, archive/signals snapshots, scale/counts, four raw artifacts and separate recomputation. The complete package suite is green (`424 passed`). Remaining before closure: commit a clean candidate, then create the actual production-shaped runner/verifier/check-only receipts.
...
- **未** — Versioned deterministic generator produces a 50+ MiB seed with 800 agents, 8,200 messages/recipients and records canonical provenance, raw SHA/size and table counts without opening production. Generator receipt SHA is now a required external rehearsal input. The integration suite is green; remaining: clean commit, then one actual full-scale rehearsal + verifier + check-only closure receipt.
